...
- **Raw-deflate scan inside opaque streams** — **PDF closed 2026-07-15 (M3k KIND_PDF):** zlib-header scan + redeflate/preflate verify + embedded-JPEG race; test.pdf −74% / arxiv.pdf −48.2% vs xz-9e, both beating precomp|xz. Remaining generalizations: (a) run the same scanner on all KIND_OPAQUE bytes (SWF, WOFF, PSD, save-game blobs) — the code is format-agnostic, only the `%PDF-` gate is PDF-specific; measure false-positive scan cost on binary corpora first; (b) PDFs *inside* containers aren't dispatched (no OP_PDF_STORE in tar/ar/zip walkers) — same mechanical extension as OP_ZIP_STORE; (c) encrypted PDFs are ciphertext and stay opaque (precomp also 0/943 on the ISO 32000 spec PDF) — not a gap, a physics limit.
