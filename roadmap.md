...
- **Encode-time hot spots** — partially closed 2026-05-07. Three pack_zst tuning changes shipped: (a) level reorder `{19,22,20,21,18,17,9,6,3,1}` → `{3,19,22,20,18,17,9,6,1,21}` so the CLI default (-3) and common high-effort (-19) levels probe first; (b) 8-probe cap on the `(level, --long)` ladder, bounding worst-case time on libzstd-direct producers (dpkg-deb, etc.) that CLI shell-out can't reproduce; (c) multi-frame fast-fail (scan past the first 28 B5 2F FD for any second occurrence — catches `-T0` worker concatenation). Wall-time: real_coreutils.deb 51 s → 19.6 s (−62%); mixed.tar.zst3 21.6 s → 3.4 s (−84%). All sizes preserved byte-exact, all RT OK. Remaining headroom: pack_xz has the same shape (10-entry preset ladder per file, ~4-7 s each) — analogous reorder + cap would help there too if real `.tar.xz` measurements surface unreachable encoders.
