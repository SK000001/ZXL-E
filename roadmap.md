...
## Current state (2026-07-14, v7 format + OP_ZIP_STORE + hardening + bench widening)

M1 + M2 + M3a–j + ZXLE_VER 3 final-step xz-9e + M5 --slow + per-fixture min-pack tier + M6 v1 + M6 v2 + M6 v3 + parser fuzz harness + M7 steps 1/2/4 + large-corpus measurement + XLSX/PPTX bench + M8a steps 1-4 (SA-based optimal-parse, gate partial pass; probes under `tools/m8/`) + **ZXLE_VER 7 (xz-compressed manifest + per-entry crc32 + 64-bit IO + OP_ZIP_STORE zip-in-tar/ar recursion) + BCJ arch guard + --fast block scaling + pack/unpack hardening + 7z & precomp-cn|xz competitor bench** ship end-to-end.

The 2026-07-14 session came from a full-repo improvements review; shipped items and their numbers are in [delivered.md](delivered.md). Surviving unshipped items are folded into "Future work" below (marked **[2026-07-14 review]**).
...
- **Multi-threaded pack** — probe ladders and min-pack tiers parallelized (M7 steps 1/2); the per-entry recompression loops (preflate/brunsli/packMP3 inside pack_zip/tar/ar) are still serial. **[2026-07-14 review]** a worker pool over entries is the natural M7 step 5; entries are independent, splice recipe+solid fragments in order.
- **Pack-time competitive with the codec floor** — currently 5–10× the comparable xz/zstd run even in default mode (probe ladders + double-pack via min-pack). Real-world adoption gate. **[2026-07-14 review]** two sub-items: (a) link liblzma/libzstd instead of shelling per probe (Windows spawn ~10 ms each; also removes decode-side tool-version fragility — same work as recording tool versions); (b) a min-pack predictor so the opaque pass is skipped when unwrap already beat the input container's own compressed size.
...
- **GNU tar widening** — base-256 size encoding, pax extended headers, sparse files, long-name records. Currently rejected → KIND_OPAQUE on real GNU-formatted tars. **[2026-07-14 review]** raised priority: `git archive` output is pax and modern GNU tar emits these by default, so a large slice of real tarballs miss the unwrap win; combined with OP_ZIP_STORE and manifest compression this is the biggest real-world coverage unlock left. (Note: plain 'x'/'L' payload entries may already walk — the hard rejections are base-256 fields and sparse layout; measure on real `git archive` output first.)
...
- **ZIP variants** — ZIP64 (large archives), encrypted (legacy ZipCrypto + AES), DEFLATE64, BZIP2-in-ZIP, LZMA-in-ZIP, prefix bytes (self-extractors). All currently fall through to KIND_OPAQUE; common in real Windows-world ZIPs. **[2026-07-14 review]** some writers emit ZIP64 records even below 4 GB; corpus-scan 50 real ZIPs to size the miss rate before building.
- **Raw-deflate scan inside opaque streams** — **[2026-07-14 review]** PDF (FlateDecode), SWF, WOFF, PSD embed zlib streams at arbitrary offsets; precomp's core trick is brute-scanning for them and applying exactly the preflate treatment zxle already has. The missing piece is only the scanner over KIND_OPAQUE bytes. PDF alone is a huge real-world class where zxle currently ties xz instead of beating it.
...
- **JPEG: packJPG/lepton vs brunsli** — **[2026-07-14 review]** measured gap: precomp's packJPG output beats our brunsli route by 0.70% on synth.jpg (116,656 vs 117,478). Try lepton and/or packJPG alongside brunsli, keep the smaller (min-pack style). Also the −1.05% JAR gap vs precomp-cn|xz (3,012 vs 3,044) is worth a diff — probably ZIP-structure handling, cheap to inspect.
...
- **Competitor benchmark — precomp, zpaq, 7z, precomp|xz closed; freearc + kanzi pending.** `tests/bench.sh` runs precomp v0.4.7, zpaq v7.15 `-m5`, 7-Zip `-mx=9 -ms=on` (2026-07-14), and the `precomp -cn | xz -9e` combo (2026-07-14) against zxle on the headline-positive fixtures, plus zpaq and 7z in the silesia baselines. 2026-07-14 signal: 7z ties xz-9e on containers (zxle +15–47%); precomp|xz is the close peer (zxle wins 5, ties 3, loses JAR −1.05% / JPEG −0.70%). freearc and kanzi remain unbenched — neither is bundled or on Windows MinGW PATH; would need a setup ship before measurement. Likely outcome on the unmeasured pair: zxle wins on container-heavy artifacts, loses to zpaq-class on long-text (already visible in the silesia zpaq baseline).
...
- **No CI, no unit tests.** **[2026-07-14 review]** everything rides on bench.sh run by hand on one Windows machine; Linux/Mac builds unverified. A GitHub Actions job (make preflate-deps && make && fixtures && bench-lite) would catch cross-platform and version-skew breakage continuously. Cheap, high leverage before any external user.
...
- **Manifest STRUCT bytes: variant (b) unexplored.** **[2026-07-14 review]** v7 compresses the manifest as a whole (variant a). Variant (b) — routing large OP_STRUCT payloads into bucket 0 so tar headers/ZIP CDs sit adjacent in the solid stream — should beat (a) on entry-heavy containers (cross-entry similarity) and would also fix the concatenated-tars degenerate shape at the root. Measure (b) against shipped (a) before building.
