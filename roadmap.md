...
**Funded order:**
1. ~~Competitor + corpus expansion bench~~ — **DONE 2026-07-18.** freearc/kanzi/xtool benched + wheels/Docker/APK/npm fixtures added. Result: kanzi and FreeArc lose everywhere (no deflate-reconstruction; FreeArc even loses to xz-9e); **xtool|xz is the only competitor that beats us anywhere** — three deflate-reproduction fixtures (PDF −0.98%, pure-py wheel −0.76%, APK −0.07%), same class as the precomp|xz hairline losses. razor never located (no public binary). The exact loss list is now item 2's spec. Numbers in delivered.md.
2. **Fix the loss list (deflate-reproduction class).** The one lever that flips every xtool/precomp loss: a **zlib parameter ladder** in the redeflate path (below). Not binary-filter work as originally guessed — razor was never benched, and the actual losses are all zlib-param reproduction, not executable filtering. Second lever (new class, not a loss but a large unclaimed win): the **Go-deflate reproducer** for the Docker-layer/Go-binary class. Both under per-stream improvements.
...
- **zlib parameter ladder in the redeflate path (Crown-A loss fix, top ratio lever).** **[2026-07-18]** The only class where any competitor beats us: PDF −0.98%, pure-py wheel −0.76%, APK −0.07% vs xtool|xz (and the earlier precomp|xz APK/express hairline losses). Root cause: when a deflate stream was produced with non-default zlib params, our fast path tries only `raw_deflate_l9` (level 9, default strategy) for an exact byte match, then falls to a preflate diff that costs bytes. xtool/precomp/razor search the zlib parameter space — (level 1–9 × strategy {default,filtered,huffman-only,RLE} × memLevel) — and find exact byte-reproductions far more often, so they store *no* diff. Fix: before the preflate fallback, run a bounded ladder of zlib re-deflates over the candidate param set and take the first exact match; only diff when the whole ladder misses. Bounded work (≤~40 re-deflates per stream, parallelizable via the existing pool), pure ratio, no format change (OP_REDEFLATE already carries no params — an exact match just means a smaller stored stream). Flips all three xtool losses to wins. Applies everywhere deflate is reproduced: zip.c, png.c, gz.c, pdf.c, the tar/ar STORE paths.
- **Go-deflate reproducer (new class: Docker layers, Go binary releases).** **[2026-07-18]** preflate cannot split Go `compress/flate` output at all (probe: split FAILED on the whole 3.4 MB alpine layer), so Docker layers and Go-built release tarballs stay KIND_OPAQUE and tie xz-9e / lose to 7z. Go's flate uses a different match-selection and token layout than zlib; a reproducer would need to model Go's encoder (or bundle a recompressor that already does — check whether xtool's zstd/oodle plugins secretly cover this, and whether `precomp`'s brute mode does). Large win on a huge real-world class (every Docker image layer on earth); unknown effort until Go's flate is characterized. Instrument first: measure how many real Docker layers are Go-flate vs gzip-classic.
...
- **Competitor benchmark — precomp, zpaq, 7z, precomp|xz, kanzi, xtool, FreeArc closed; razor still pending.** **[2026-07-18]** `tests/bench.sh` runs precomp v0.4.7, zpaq v7.15 `-m5`, 7-Zip `-mx=9 -ms=on`, `precomp -cn | xz -9e`, **kanzi-cpp 2.5.3 `-l 9`, xtool 0.7.9 `precomp -mzlib | xz -9e`, and FreeArc 0.67 `-mx`** (last three Windows-only, `make kanzi-deps`/`xtool-deps`). Verdict: kanzi and FreeArc lose to zxle on every container/real-world fixture (no deflate-reconstruction; FreeArc even loses to xz-9e). **xtool|xz is the only competitor that beats us anywhere** — PDF −0.98%, pure-py wheel −0.76%, APK −0.07%, all in the zlib-param-reproduction class (the ladder above is the counter). **razor still unbenched** — Christian Martelock's razor has no public download that a websearch surfaced; it's the last serious repacker-class tool untested. It shares xtool's deflate-reproduction approach plus its own strong LZ, so expect the same loss shape (zlib-param class) rather than a new one; the ladder likely covers it too. Get a binary if one surfaces, else this stays a documented gap. Long text still loses to zpaq-class (silesia baseline; text middle tier is the counter).
