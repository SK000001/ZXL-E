...
## Current state (2026-07-17, M3l PDF-in-container; clean competitor sweep holds)

M1 + M2 + M3a–j + M3k (KIND_PDF FlateDecode scanner) + **M3l (PDF-in-container: pack_pdf dispatch in tar/ar/zip walkers, rides OP_ZIP_STORE, no format bump)** + ZXLE_VER 3 final-step xz-9e + M5 --slow + per-fixture min-pack tier + M6 v1 + M6 v2 + M6 v3 + parser fuzz harness + M7 steps 1/2/4 + large-corpus measurement + XLSX/PPTX bench + M8a steps 1-4 (probes under `tools/m8/`) + ZXLE_VER 7 (xz-compressed manifest + per-entry crc32 + 64-bit IO + OP_ZIP_STORE + merged-manifest layout) + BCJ arch guard + --fast block scaling + pack/unpack hardening + 7z & precomp-cn|xz competitor bench + JPEG codec race ship end-to-end.
...
- **Raw-deflate scan inside opaque streams** — **PDF closed 2026-07-15 (M3k KIND_PDF):** zlib-header scan + redeflate/preflate verify + embedded-JPEG race; test.pdf −74% / arxiv.pdf −48.2% vs xz-9e, both beating precomp|xz. **(b) PDFs inside containers closed 2026-07-17 (M3l):** tar/ar walkers + stored ZIP entries dispatch pack_pdf, nested recipe rides OP_ZIP_STORE (no format bump); pdf-in.tar −24.3% / zip-with-pdf.zip −34.0% vs xz-9e, both beating precomp|xz and 7z. Remaining generalizations: (a) run the same scanner on all KIND_OPAQUE bytes (SWF, WOFF, PSD, save-game blobs) — the code is format-agnostic, only the `%PDF-` gate is PDF-specific; measure false-positive scan cost on binary corpora first; (c) encrypted PDFs are ciphertext and stay opaque (precomp also 0/943 on the ISO 32000 spec PDF) — not a gap, a physics limit; (d) PDFs inside *deflated* ZIP entries aren't dispatched (inflated bytes go to solid via REDEFLATE/PREFLATE; nesting a recipe under those ops is the recursive `dispatch_stream()` end-state above).
...
- **pack_zip can orphan solid bytes on hostile input.** **[2026-07-17 note]** `pack_zip` has one mid-walk `return -1` (stored entry whose CD raw_size ≠ comp_size — malformed/hostile ZIP) that fires *after* earlier entries may have appended to b0/b1. Callers (tar/ar walkers, top-level dispatch) don't snapshot/restore bucket lengths, so the fallback path packs with orphaned bytes in the solid stream → desynced ops → broken archive, caught only at unpack by the v7 crc (fails hard, not silent). Fix is cheap: save `b0->n`/`b1->n` before nested pack_* calls and truncate on failure — or make pack_zip validate all entries before its first append. `pack_pdf` is already rollback-safe (returns −1 only before any append). v7 (2026-07-14) added per-entry crc32 verified at unpack — corruption or decode-side tool-version drift now fails hard instead of silently emitting wrong bytes. Recovery still absent (solid stream: one bad byte loses everything after it); per-entry framing would cost ratio, so document rather than build.
