...
## Current state (2026-07-23c, --best kanzi tier shipped — after v8 ladder + v9 multi-member gzip)
...
- ~~**Multi-member gzip**~~ — **SHIPPED 2026-07-23 (v9).** BGZF (.bam/.bgz genomics — concatenated gzip *by spec*), klauspost pgzip, concatenated logs now unwrap per-member (KIND_GZIP recipe gains a u32 member count). Fixture `multi.gz` −42.6% vs xz-9e, 121/121 RT OK, no regressions; details in [delivered.md](delivered.md). **Multi-member bzip2** still open (rare; add only if a real corpus hits it).
