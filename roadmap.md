...
## Current state (2026-07-15, JPEG codec race + merged-manifest layout; clean competitor sweep)

M1 + M2 + M3a–j + ZXLE_VER 3 final-step xz-9e + M5 --slow + per-fixture min-pack tier + M6 v1 + M6 v2 + M6 v3 + parser fuzz harness + M7 steps 1/2/4 + large-corpus measurement + XLSX/PPTX bench + M8a steps 1-4 (SA-based optimal-parse, gate partial pass; probes under `tools/m8/`) + ZXLE_VER 7 (xz-compressed manifest + per-entry crc32 + 64-bit IO + OP_ZIP_STORE) + BCJ arch guard + --fast block scaling + pack/unpack hardening + 7z & precomp-cn|xz competitor bench + **JPEG codec race (brunsli vs packJPG, smaller wins) + merged-manifest layout (manifest rides in bucket 0's xz stream; small-input layout race)** ship end-to-end.

**As of 2026-07-15, no benched competitor produces a smaller archive than zxle on any bench fixture** — zxle beats `precomp -cn | xz -9e` on all 10 combo fixtures (+0.02% to +7.42%), 7z -mx=9 by 15–47%, zpaq -m5 per-fixture on containers. Remaining "best" gaps are unmeasured competitors (freearc, kanzi, xtool/razor-class) and coverage classes where we go opaque (PDF, pax tars, ZIP64 — see below). The 2026-07-14/15 sessions came from a full-repo improvements review; shipped items and numbers are in [delivered.md](delivered.md); surviving items are marked **[2026-07-14 review]** below.
