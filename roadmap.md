...
## Current state (2026-07-17b, M3l + M3m + bucket rollback + CI + M7 step 5 zip pool)
...
- **Multi-threaded pack** — probe ladders and min-pack tiers parallelized (M7 steps 1/2); **pack_zip's entry loop parallelized 2026-07-17 (M7 step 5)** — the tar/ar entry loops (whole nested container packs per entry) are the remaining serial recompression path.
...
5. ~~**Worker pool over per-entry recompression loops**~~ — **zip shipped 2026-07-17 (M7 step 5).** pack_zip's entry payloads (inflate + redeflate/preflate + nested png/jpeg/pdf) run on a pthread pool into per-entry fragments, spliced in order — output byte-identical. pe-deflate-l6 pack −36%, pptx −38%, docx only −8% (one dominant entry). libpreflate concurrency-hammered clean first. **Remaining: tar/ar entry loops** (entries dispatch whole nested container packs; same fragment-splice pattern, wants its own measurement).
