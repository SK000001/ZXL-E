...
**2026-07-23 deep-research pass (no code shipped; plan rewritten):** four web-research rounds + one live experiment. Headlines: **microsoft/preflate-rs supersedes the planned in-house zlib ladder** (which was also mis-costed — it needed a v8 wire bump); **git packfiles are a free win class** (measured: −28.5% vs xz-9e with the existing M3m scanner, zero new code, RT OK); razor binaries located (Ultra F-A bundle); libbsc chosen as the text-middle-tier vehicle; M8 re-scoped around LzmaEnc's pluggable match-finder interface; OpenZL (Meta, Oct 2025) identified as complement-not-rival and a bucket-0 backend candidate for structured raw payloads. Strategic program below rewritten to **funded order v4**; per-stream and coverage sections updated with **[2026-07-23]** tags.
...
2. **preflate-rs probe → integrate (supersedes the hand-rolled zlib ladder).** [microsoft/preflate-rs](https://github.com/microsoft/preflate-rs) (Apache-2.0, v0.7.6 Mar 2026, actively maintained, C FFI, fuzzed, no-unsafe) fingerprints the origin compressor — hash algo, chain depth, nice-length, window, block-splitting — across **zlib all-levels / zlib-ng / libdeflate / miniz / Windows zlib**, typical overhead <1%. The ladder we'd planned covers only classic zlib *and* was mis-costed: it needs a v8 wire bump (OP_REDEFLATE decode re-runs fixed L9 params — recipe.c:33 — so non-default params must ride in the recipe). preflate-rs corrections ride the existing OP_PREFLATE diff slot; the diff *format* changes, so integration still needs a version gate. **Probe first (metric fixed):** correction-size histogram over our corpus's OP_PREFLATE streams; xtool parity = ≤3 B corrections on classic-zlib streams (encode.su: old preflate leaves 20–30% of streams above that; precomp/xtool brute-force hits it). Also probe the alpine Go-flate layer: preflate-rs takes *unknown* compressors with fat-but-bounded corrections, and the win condition is only `xz(inflated) + corrections < gzip_size` — a ~30% budget on a 2:1-deflated layer. Go-flate may partially fall without a bespoke reproducer.
...
- **preflate-rs integration (Crown-A loss fix, top ratio lever; supersedes the zlib ladder 2026-07-23).** **[2026-07-18, rewritten 2026-07-23]** The only class where any competitor beats us: PDF −0.98%, pure-py wheel −0.76%, APK −0.07% vs xtool|xz. Root cause: non-default-param deflate streams fall past our `raw_deflate_l9` fast path to a preflate diff that costs bytes; xtool/precomp brute-search zlib params and store ~no diff (≤3 B typical). The originally-planned in-house ladder was mis-costed — "no format change" was wrong: OP_REDEFLATE's decode side re-runs *fixed* L9 params (recipe.c), so any non-default exact match needs params in the recipe → v8 wire bump — and it would never cover zlib-ng/libdeflate/miniz streams. **[microsoft/preflate-rs](https://github.com/microsoft/preflate-rs)** (Apache-2.0, actively maintained, C FFI, <1% typical overhead) fingerprints all of those *plus* Windows zlib, and its corrections ride the existing OP_PREFLATE diff slot. Costs to carry: correction format differs from deus-libri preflate → decode coupled to packing library → version-gate the swap (crc already fails hard on mismatch); Rust static lib enters the build/CI. **Probe before integrating** (1 session): correction-size histogram on our OP_PREFLATE streams — target ≤3 B on classic zlib (xtool parity) — plus the alpine Go-flate layer vs its correction budget, plus the 96 preflate streams in the git-packfile fixture. Applies everywhere deflate is reproduced: zip.c, png.c, gz.c, pdf.c, tar/ar STORE paths.
- **Go-deflate reproducer (new class: Docker layers, Go binary releases).** **[2026-07-18, re-scoped 2026-07-23]** preflate (2018 C++) cannot split Go `compress/flate` output at all (probe: split FAILED on the whole 3.4 MB alpine layer), so Docker layers and Go-built release tarballs stay KIND_OPAQUE and tie xz-9e / lose to 7z. **Harder than assumed:** there are *two* Go encoder families in the wild — std `compress/flate` (levels 1–3 own fast encoder, 4–9 own lazy matcher; docker-save-era layers) and klauspost/compress flate (levels 1–6 multiple fastEnc variants, 7–9 lazy, opportunistic Huffman table reuse, *changes between releases*; BuildKit and most modern Go tooling) — plus klauspost pgzip (independent gzip members per block, which actually helps). A bit-exact reproducer means modeling both families across versions: substantial, ongoing burden. **Unfunded until the preflate-rs probe (funded item 2) answers two questions:** (a) do fat-but-bounded corrections already clear the budget (`xz(inflated)+corrections < gzip_size` — ~30% of stream on a 2:1-deflated layer)? (b) does a layer-corpus scan show enough std-Go vs klauspost vs classic-zlib mass to justify bespoke work? Counter-trend: OCI v1.1 zstd layers are coming (gzip still default everywhere), and BuildKit's zstd is klauspost pure-Go with **only 4 effective levels, frozen by policy** — the future zstd-layer class is a trivial ladder row, not a research project.
