...
## Current state (2026-07-29, v10 gz-member ladder — after --best kanzi tier + v8 ladder + v9 multi-member gzip)
...
2. ~~**Cheap zlib ladder — the loss fix**~~ — **SHIPPED 2026-07-23 (v8, `OP_REDEFLATE_P`).** The instrument-first probe (throwaway branch `feat/preflate-rs-probe`, abandoned) settled this against the round-2 preflate-rs recommendation. Per-fixture stream histogram of the current redeflate/preflate/store decision, then a grid-search asking "does any stock zlib param set exact-reproduce each stream that currently pays a preflate diff?":
   ...
   **Every measured xtool *loss* was plain zlib level-6** (memLevel 8/9, wbits −15; a few L4/L5) — our L9-only fast path missed it and fell to a diff. **Shipped as v8 `OP_REDEFLATE_P`** (two param bytes encoding level/memLevel/strategy/windowBits; decode re-deflates at them). Flipped every loss: requests.whl −404 B, newpipe.apk −9,932 B, test.pdf −420 B (now beats precomp+xz), git packfile −7,776 B (96 preflate → exact); arxiv.pdf's 40 non-zlib streams correctly stay preflate. Full bench 121/121 RT OK, solid 0.3174, per-file 0.3232 flat, no regressions. Details in [delivered.md](delivered.md). Deferred within the milestone: png.c/gz.c keep their own recipe layouts (no measured loss); filtered/huffman strategies and memLevel <8 aren't in the search set (0 need on measured data), but the encoding stores actual params so the set can widen with no wire change.
...
3. ~~**Text middle tier**~~ — **SHIPPED 2026-07-23 as `--best` (kanzi -l9).** kanzi was already built (competitor), so zero new dependency. Measured a pareto point between default (xz) and --slow (zpaq): dickens −24.4% vs xz in ~2s, +2.2% behind zpaq-m5 at ~15× the speed. Wins across text/source/OOXML: docx −34.35%, webster −25.99%, dickens −24.40%, samba −16.51% vs default, all RT OK; default/--slow byte-identical (122/122 RT OK). Details in [delivered.md](delivered.md). **Remaining tier work:** (a) auto-race kanzi in *default* mode (not just the `--best` flag) — captures the win without opt-in but touches the merged-manifest finalize; (b) **libbsc** (3.3.12, Apache-2.0, CUDA BWT) stays the alternative vehicle if a kanzi limitation surfaces; bsc-m03 (enwik8 20.49 MB) is the BWT ratio ceiling; bzip3 stays a bench reference (LGPL, CVE history).
...
5. **Cheap ratio stack (mostly zero/near-zero code).** (a) **jbrd third JPEG racer**: `cjxl --lossless_jpeg` transcodes JPEG→JXL bit-exactly (jbrd reconstruction data; ~20% smaller; brunsli's successor inside VarDCT) — u8 codec byte has room. (b) **git packfiles — measured 2026-07-23, zero new code:** kanzi clone pack 421,613 → zxle 301,664 (**−28.5%**; 171 zlib streams: 75 redeflate + 96 preflate; RT OK) while xz-9e ties at +0.0% — packfiles are wall-to-wall zlib and opaque to every conventional tool; add a bench fixture. (c) ~~**BGZF/BAM** multi-member gzip~~ — **SHIPPED 2026-07-23 (v9);** fixture −42.6% vs xz-9e. **v9 members used L9/preflate only; the v8 redeflate ladder was wired into gz members 2026-07-29 (v10)** so level-6 BGZF/pgzip members exact-redeflate instead of paying a preflate diff (multi.gz −16 B; ~11.5 B/member on an all-L6 fixture, scales with member count). (d) **TorrentZip** ROM sets: canonical fixed deflate params by spec — existing redeflate likely hits 100%; verify locally. (e) **Minecraft .mca**: zlib-per-chunk region files — M3m should fire as-is; verify locally. **(a) jbrd JPEG racer is blocked on a cjxl/libjxl binary** (no fetchable Windows build wired; source build is heavy) — deferred until a binary is available.
...
- ~~**Multi-member gzip**~~ — **SHIPPED 2026-07-23 (v9); gz-member redeflate ladder added 2026-07-29 (v10).** BGZF (.bam/.bgz genomics — concatenated gzip *by spec*), klauspost pgzip, concatenated logs now unwrap per-member (KIND_GZIP recipe gains a u32 member count). Fixture `multi.gz` −42.6% vs xz-9e, 122/122 RT OK, no regressions; details in [delivered.md](delivered.md). v10 wires the v8 ladder into gz members so level-6 members exact-redeflate (`mode=2` + 2 param bytes) instead of a preflate diff. **Multi-member bzip2** still open (rare; add only if a real corpus hits it).
...
- ~~**zlib ladder — Crown-A loss fix**~~ — **SHIPPED 2026-07-23 (v8 `OP_REDEFLATE_P`).** **[2026-07-18, probe + ship 2026-07-23]** The only class where any competitor beat us: PDF −0.98%, pure-py wheel −0.76%, APK −0.07% vs xtool|xz. Probe root-caused it as plain zlib **level-6** (Python zipfile, Android, git), not zlib-ng/libdeflate/Go: a stock-zlib grid-search exact-reproduced 12/12 wheel, 16/16 APK, 4/4 test.pdf, 96/96 git-packfile preflate streams (winners cluster at level 6, default strategy, memLevel 8/9, wbits −15). Shipped: a bounded in-house ladder (`redeflate_ladder_find`) tried before the preflate fallback, storing two param bytes in `OP_REDEFLATE_P`. Flipped every loss (wheel −404, APK −9,932, test.pdf −420, packfile −7,776; arxiv unchanged); 121/121 RT OK, no regressions. Wired into zip.c + pdf.c (generic recipe path); png.c/gz.c deferred (no measured loss). **What the ladder does NOT cover (→ [microsoft/preflate-rs](https://github.com/microsoft/preflate-rs), funded item 2b):** non-zlib producers — arxiv.pdf's 40 pdfTeX/GS streams were 0/40 ladder-MISS (already a win) — and Go flate (alpine layer preflate `split=0`; only preflate-rs's unknown-compressor mode might crack it, unproven until built).
...
- **Text / source code** — default xz-9e, `--best` kanzi -l9 (2026-07-23), `--slow` zpaq-m5. **[2026-07-14 review; middle tier CLOSED 2026-07-23]** the default↔slow silesia gap (17%: 0.2269 vs 0.1891) is now filled by `--best` (kanzi -l9): dickens −24.4% vs xz at ~15× zpaq's speed, +2.2% behind zpaq-m5. Original review continues: Also worth one bench: with --slow, bucket 1 still goes to xz+BCJ — zpaq -m5 has an internal E8/E9 model and may beat it on PE bytes. **[2026-07-23]** Vehicle decided: **libbsc** (3.3.12 Sep 2025, Apache-2.0, AVX512 + CUDA BWT/IBWT — 881 MB/s enwik10 GPU decode — same author as the libsais/libcubwt we already ship). bzip3 demoted to bench-reference (LGPL, 2023 CVE cluster, upstream data-loss disclaimer); bsc-m03 = ratio ceiling reference (best BWT known, enwik8 20.49 MB, GPL/experimental); also bench 7z-PPMd and durilca (binary in the Ultra F-A bundle) on the text bucket. Silesia anchors for the tier: xz-9e ≈48.4 MB, zpaq-m5 39.1 MB, precomp|cmix 28.26 MB (all-time #5), paq8px 27.83 MB (ceiling).
