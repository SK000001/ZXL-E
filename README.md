...
## Status (2026-07-29 — v8 redeflate ladder + v9/v10 multi-member gzip + `--best` kanzi tier)
...
**Multi-member gzip (v9 + v10):** BGZF (`.bam` genomics, concatenated gzip *by spec*), klauspost pgzip (some Docker layers), and concatenated logs previously fell whole to opaque; `pack_gz` now unwraps every member (`−42.7%` vs xz-9e on the `multi.gz` fixture). **v10** wires the v8 redeflate ladder into gz members, so level-6 members (the BGZF/pgzip default) exact-redeflate — storing two zlib param bytes instead of a costlier preflate diff — rather than paying a diff per member (multi.gz 44,120 → 44,104 B; ~11.5 B saved per L6 member, scales with member count). Single-member gzips are unchanged.
...
| **v10 gz-member redeflate ladder** | **shipped** — the v8 ladder wired into gzip members (`mode=2` + 2 param bytes); level-6 BGZF/pgzip members exact-redeflate instead of a preflate diff (multi.gz 44,120→44,104 B, ~11.5 B/member on all-L6, scales with member count). ZXLE_VER 9→10, 122/122 RT OK, solid/per-file byte-identical, no regressions |

## Headline numbers (2026-07-29, ZXLE_VER 10)
...
| multi.gz (multi-member gzip fixture, v10 gz-member ladder) | — | **−42.7%** (44,104 B vs xz-9e 76,916) | — |
