// M8a step 4 candidate v7 -- v6 + per-match-length-class FSE cost.
//
                                          ...
                                          const uint32_t *ml_fse_cost_arr)
{
    ...
    return of_fse_cost_arr[oc] + ml_fse_cost_arr[lc] + (oc + lc) * 8 + 16;
}
    ...
    parse_record_t *recs, long recs_cap)
{
    ...
    for (long i = 0; i < n; i++) {
        ...
        for (int k = 0; k < idx[i].count; k++) {
            ...
            else if (of_fse_cost_arr && ml_fse_cost_arr) add = match_cost_learned(L, O, of_fse_cost_arr, ml_fse_cost_arr);
...
int main(int argc, char **argv) {
    ...
    long nrec1 = optimal_parse_pass(n, T, idx, pool, NULL, NULL, NULL, recs, n + 1);
    ...
    uint32_t ml_fse_cost[M8_ML_CLASSES];
    build_ml_cost(recs, nrec1, ml_fse_cost);
    {
        uint32_t olo = UINT32_MAX, ohi = 0, mlo = UINT32_MAX, mhi = 0;
        for (int c = 0; c < M8_OF_CLASSES; c++) {
            if (of_fse_cost[c] < olo) olo = of_fse_cost[c];
            if (of_fse_cost[c] > ohi) ohi = of_fse_cost[c];
        }
        for (int c = 0; c < M8_ML_CLASSES; c++) {
            if (ml_fse_cost[c] < mlo) mlo = ml_fse_cost[c];
            if (ml_fse_cost[c] > mhi) mhi = ml_fse_cost[c];
        }
        printf("of_cost    : min=%.2fb max=%.2fb (per offset class)\n", olo / 8.0, ohi / 8.0);
        printf("ml_cost    : min=%.2fb max=%.2fb (per ml class)\n",     mlo / 8.0, mhi / 8.0);
    }

    double tp2_0 = now_sec();
    long nrec = optimal_parse_pass(n, T, idx, pool, byte_cost, of_fse_cost, ml_fse_cost, recs, n + 1);
    ...
    printf("---\n");
    printf("M8a v7 (om) : %.4f  (%zu / %ld)\n", m8_ratio, zsize, n);
    printf("zstd-19     : %.4f  (%zu / %ld)\n", ref_ratio, ref_size, n);
    printf("delta       : %+.2f%% (M8a v7 vs zstd-19)\n", delta);
