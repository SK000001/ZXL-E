// M8a step 2 -- optimal-parse with REPCODE tracking in a forward DP.
//
// v2 used backward DP over multi-length candidates; closed the gap to zstd-19
// to +1.25% at 1 MB (was +1.55% in v1). Remaining gap comes from a cost model
// that prices every match offset at log2(offset), even when the offset is a
// repeat of one of the last 3 used -- in zstd's FSE coding such offsets encode
// as a tiny offset code (~2-3 bits) instead of the full log2(offset).
//
// v3 adds:
//   - Forward DP carrying state = (cost, rep[3]) at each position.
//   - At each position try: literal, every (length, offset) candidate, and a
//     live scan of T at each of the 3 current rep offsets (smallest_offset
//     candidate collection in v2 might not preserve a rep-matching offset).
//   - Cost model gives rep matches a cheap fixed offset cost (~3 bits).
//
// Output sequences with raw offsets and rep=0; ZSTD_compressSequences at
// level >= 10 re-detects repcodes internally, so a parse that selects more
// rep-reusable matches encodes smaller.
...
// Rep match: tiny fixed offset code (~3 bits) + length cost.
static inline uint32_t rep_cost(uint32_t l)                 { return (3 + log2_u32(l)) * 8; }

// State at each position: best cost to reach + the rep[3] active there + back-pointer.
typedef struct {
    uint64_t cost;
    uint32_t rep[3];
    uint32_t match_len;   // 0 = literal step; else this was a match of (len, off)
    uint32_t match_off;
    uint32_t prev_pos;    // back-pointer
} fstate_t;

typedef struct { uint32_t lit_len; uint32_t match_len; uint32_t match_off; } parse_record_t;

// Try transition i -> j with given match (or 0 for literal) and update repcode state.
                            ...
                            uint32_t m_len, uint32_t m_off)
{
    ...
    if (m_len == 0) {
        // Literal: rep state unchanged.
        F[j].rep[0] = F[i].rep[0];
        F[j].rep[1] = F[i].rep[1];
        F[j].rep[2] = F[i].rep[2];
    } else {
        // Match: check if m_off hits one of the reps.
        if (m_off == F[i].rep[0]) {
            F[j].rep[0] = F[i].rep[0];
            F[j].rep[1] = F[i].rep[1];
            F[j].rep[2] = F[i].rep[2];
...
static long optimal_parse_forward_rep(
    ...
    parse_record_t *recs, long recs_cap)
{
    ...
    F[0].cost = 0;
    // zstd initial repcodes are 1, 4, 8.
    F[0].rep[0] = 1; F[0].rep[1] = 4; F[0].rep[2] = 8;

    for (long i = 0; i < n; i++) {
        if (F[i].cost == UINT64_MAX) continue;

        // Literal step (advance 1).
        try_step(F, i, i + 1, lit_cost(), 0, 0);

        // Multi-length candidates from SA walk.
        ...
        // Live-scan the 3 rep offsets; the candidate set may not include them
        // (we keep smallest_offset per length tier, not rep-matching offset).
        for (int k = 0; k < 3; k++) {
            ...
            if (L >= M8_MINMATCH) {
                try_step(F, i, i + (long)L, rep_cost((uint32_t)L), (uint32_t)L, O);
            }
        }
    }
    ...
    // Backtrack: collect choices from n back to 0.
    long max_steps = n;
    uint32_t *step_match_len = (uint32_t *)malloc(sizeof(uint32_t) * (size_t)(max_steps + 1));
    uint32_t *step_match_off = (uint32_t *)malloc(sizeof(uint32_t) * (size_t)(max_steps + 1));
    long    *step_pos        = (long    *)malloc(sizeof(long)     * (size_t)(max_steps + 1));
    long ns = 0;
    {
        long p = n;
        while (p > 0) {
    ...
    // Reverse iterate: walk forward through steps.
    long nrec = 0;
    long lit_start = 0;
    long i = 0;
    for (long s = ns - 1; s >= 0; s--) {
        ...
        if (ml == 0) {
            // Literal step: just advance.
            i = end;
        } else {
            // Emit a sequence: (i - lit_start) literals then a match.
            if (nrec >= recs_cap) {
...
int main(int argc, char **argv) {
    ...
    double td0 = now_sec();
    long nrec = optimal_parse_forward_rep(n, T, idx, pool, recs, n + 1);
    double td1 = now_sec();
    ...
    printf("---\n");
    printf("M8a v3 (rep): %.4f  (%zu / %ld)\n", m8_ratio, zsize, n);
    printf("zstd-19     : %.4f  (%zu / %ld)\n", ref_ratio, ref_size, n);
    printf("delta       : %+.2f%% (M8a v3 vs zstd-19)\n", delta);
