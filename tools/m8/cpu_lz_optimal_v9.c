// M8a step 4 candidate v9 -- v6 + wider candidate enumeration.
//
...
#define M8_MAX_CANDS 16     // v9: 8 -> 16
#define M8_WALK_LIMIT 512   // v9: 128 -> 512
    ...
    parse_record_t *recs, long recs_cap)
{
    ...
    for (long i = 0; i < n; i++) {
        ...
        for (int k = 0; k < idx[i].count; k++) {
            ...
            else if (of_fse_cost_arr) add = match_cost_learned(L, O, of_fse_cost_arr);
...
int main(int argc, char **argv) {
    ...
    printf("---\n");
    printf("M8a v9 (wid): %.4f  (%zu / %ld)\n", m8_ratio, zsize, n);
    printf("zstd-19     : %.4f  (%zu / %ld)\n", ref_ratio, ref_size, n);
    printf("delta       : %+.2f%% (M8a v9 vs zstd-19)\n", delta);
