# ZXL-E improvement review — triage record (reviewed 2026-07-14; executed 2026-07-14/15)
...
| # | Item | Disposition |
|---|---|---|
| 1.1 | Manifest/recipes stored uncompressed | **Shipped** (v7 whole-manifest xz, 2026-07-14; merged-manifest layout + small-input layout race, 2026-07-15 — captures variant (b)'s main win). Only STRUCT *placement* remains, noted in roadmap |
...
| 1.10 | packJPG/lepton vs brunsli; varints; PNG unfilter | **packJPG shipped 2026-07-15** (codec race; synth.jpg now beats precomp\|xz). lepton/varints/PNG-unfilter deferred → roadmap |
...
Headline outcomes of the executed items: sample.jar 6,260 → 2,804 (−81.78% vs xz-9e;
beats precomp+xz's 3,012), synth.jpg beats precomp+xz via the packJPG race, pptx −2.8%,
mixed.tar family −1%, zip-in.tar −5.69% vs xz-9e (new capability), --fast at 1 GB +0.58%
size cost (was +3.24%), and unpack now fails hard on corruption or tool-version drift
instead of silently emitting wrong bytes. **After the 2026-07-15 ships, no benched
competitor produces a smaller archive than zxle on any bench fixture** (precomp|xz loses
all 10 shared fixtures; 7z loses by 15–47%). 66/66 bench fixtures RT OK.
