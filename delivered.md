...
3. **GitHub Actions CI** (`.github/workflows/ci.yml`): ubuntu build of libpreflate + zxle, self-contained fixtures (docx/xlsx/pptx/jar/flate-blob), hostile.sh, bench gated on no rt=FAIL. Portability fixes it forced: preflate-deps no longer hardcodes MinGW cmake generator/mingw32-make (OS-conditional, `cmake --build`); make_fixtures.sh no longer dies under `set -e` when ffmpeg is absent. **Pushed 2026-07-17: first run caught a real Linux bug (upstream libpreflate hardcodes MSVC _ftelli64/_fseeki64 — now patched in preflate-deps, error-swallowing || true removed); second run green end-to-end.**
...
### M7 step 5 — worker pool over pack_zip's entry loop (shipped 2026-07-17; tar/ar completed same day)
...
- **tar/ar completion (same day):** pack_tar/pack_ar restructured to the same parse→pool→splice shape; the parse phase validates every header before any output exists, making the hostile-input rollback structural (no snapshot needed). Measured: gz-in.tar −13%, mixed.deb −12%, mixed.tar −9%, mixed.tar.gz −7%, wrapped-tar family −2..−5%; real_coreutils pair flat (single dominant entry / ladder-bound). With this every per-entry recompression loop is parallel. **Remaining pack-time floor:** the final `xz -9e --threads=1` over the solid payload (the price of the ratio — `--fast` trades it at +0.6–2.8%) and the wrapper-reproduction ladders (parallel since step 1, wall-bounded by their slowest probe — real_coreutils_src.tar.xz's ~27 s is one xz -9e re-encode of a ~40 MB body plus the final solid pass). Also the bench itself now caches all deterministic competitor/baseline work (2026-07-17, `tests/baseline/.cache`): 292 s → 172 s warm, and the remaining 172 s is zxle's own measured pack/unpack.
